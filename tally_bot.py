...
TALLY_ORG_ID = os.getenv('TALLY_ORG_ID')

if not TALLY_ORG_ID:
    raise ValueError("Missing TALLY_ORG_ID. Set your target Tally organization ID in the .env file.")
...
def fetch_proposals_from_tally(limit=20, status_filter=None):
    """Fetch governance proposals directly from Tally API"""
...
class TallyProposal:
    def __init__(self, proposal_data):
        ...
        self.governor_name = governor.get('name', 'Unknown Governor')
        self.governor_slug = governor.get('slug') or governor.get('id') or 'governor'
        # Normalize common governor slug patterns (e.g., suffixes like "-governor-1")
        if self.governor_slug.endswith('-governor-1'):
            self.governor_slug = self.governor_slug.replace('-governor-1', '')
...
async def fetch_proposals():
    ...
    return await loop.run_in_executor(None, fetch_proposals_from_tally)
